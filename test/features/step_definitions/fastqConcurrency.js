...
Given("{int} messages are sent to the SQS queue", async (messageCount) => {
  ...
  if (size >= minRequired) {
    ...
    strictEqual(
      size,
      messageCount,
      `Expected at least ${minRequired} messages in queue, but found ${size} after ${attempts} attempts. LocalStack appears to be dropping messages.`,
    );
  }
});
...
When(
  "the consumer with concurrency {int} processes the messages",
  { timeout: 15000 },
  async (concurrency) => {
...
When(
  "the consumer starts with concurrency {int}",
  async (initialConcurrency) => {
...
When(
  "the concurrency is updated to {int} during processing",
  { timeout: 15000 },
...
When(
  "the consumer with concurrency {int} starts processing",
  async (concurrency) => {
    currentConsumer = fastqConsumer(concurrency);
    currentConsumer.start();
...
When(
  "the consumer without fastq options processes the messages",
  { timeout: 15000 },
  async () => {
...
Then(
  "all messages should be consumed without error",
  { timeout: 10000 },
  async () => {
    currentConsumer.stop();
...
Then("messages should be processed with controlled concurrency", () => {
  ...
  ok(
    ...
    `Max concurrent processing was ${maxConcurrentProcessing}, expected <= 2`,
  );
...
Then("no messages should be lost or processed multiple times", () => {
  ...
  strictEqual(
    uniqueIds.length,
    testMessages.length,
    "No messages should be processed multiple times",
  );
});

Then(
  "the first and third messages should be processed successfully",
  async () => {
    currentConsumer.stop();
    ...
    strictEqual(
      ...
      2,
      "Should have processed 2 messages successfully",
    );
...
Then("the updated concurrency should be reflected", () => {
  strictEqual(
    currentConsumer.concurrency,
    2,
    "Concurrency should be updated to 2",
  );
});
...
Then("the consumer should stop gracefully", () => {
  strictEqual(
    currentConsumer.status.isRunning,
    false,
...
Then("processing should use legacy Promise.all approach", () => {
  ...
  strictEqual(
    currentConsumer.processConcurrentMessages,
    false,
    "Should not use concurrent message processing",
  );
  strictEqual(
    currentConsumer.concurrency,
    null,
    "Concurrency should be null in legacy mode",
  );
});

When(
  "the consumer with concurrency {int} processes messages continuously",
  { timeout: 20000 },
  async (concurrency) => {
    ...
    ok(
      processedCount >= minRequired,
      `Should process at least ${minRequired} messages, processed ${processedCount}`,
    );
...
Then("messages should be processed with continuous polling behavior", () => {
  ...
  ok(
    pollTimestamps.length > 1,
    `Should have multiple polls, got ${pollTimestamps.length}`,
  );
  ...
  ok(
    fastPolls > 0,
    `Should have at least one fast poll (< 1s), indicating continuous polling. Got ${fastPolls} fast polls out of ${pollIntervals.length} total intervals`,
  );
});
